...
export default function ShareButtons({ title }: ShareButtonsProps) {
  ...
  const shareLinks = [
    ...
    },
    {
      ...
      bg: 'bg-[#1877F2] hover:bg-[#1465cc] text-white',
    },
  ];
    ...
    <div className="flex items-center gap-2 flex-wrap">
      ...
      {shareLinks.map((s) => (
        <a
          ...
          title={s.label}
          className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${s.bg}`}
        >
          {s.icon}
        </a>
      ))}
      ...
      {copied && <span className="text-[10px] text-green-600 font-medium">Tersalin!</span>}
    </div>
  );
}
